      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// Use previous path for smooth path planning
			...
			bool too_close = false; // Initialize the closeness 
			vector<bool> lanemark={true, true, true}; // Initialize the 3 lane open
			lanemark[mylane] = false; // Close my current lane 
			// Initialize the cost variable for left turn and right turn 
			int traffic_l = 0;
			int traffic_r = 0;

			// Looking for the status of other vehicles 
			for (int i=0; i<sensor_fusion.size();i++)
			{
				...
				int check_car_lane = lanenumber(d);

				// Calculate the Frenet S coordinate of other vehicles
				check_car_s += ((double)prev_size*0.02*check_speed);
				// Check the closeness with my front vehicle 
			    ...
			    if ((check_car_lane == 0) && (check_car_s > car_s) && (check_car_s - car_s) >= 23 && (check_car_s - car_s) < 60) {
					traffic_l += 1;
				}
			    if ((check_car_lane == 2) && (check_car_s > car_s) && (check_car_s - car_s) >= 23 && (check_car_s - car_s) < 60) {
				    traffic_r += 1;	
				}
			}
